...
import { FaBars, FaHandsHelping, FaHome, FaPlusCircle, FaSignOutAlt, FaTimes, FaUserCog, FaUsers, FaVideo } from 'react-icons/fa';
...
const Navbar = () => {
    ...
    return (
      <header className="border-b border-dark-200 bg-white">
        <div className="container-custom flex h-16 items-center justify-between">
          <Link to="/" className="flex items-center gap-2" aria-label="RW0448 UEBR BUSOGO"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary-600 text-white"><FaHandsHelping /></span><span className="text-xl font-bold text-primary-700">RW0448 </span></Link>
  ...
  return (
    <>
      ...
      <div className="fixed left-0 right-0 top-0 z-40 flex h-16 items-center justify-between gap-3 border-b border-dark-200 bg-white px-4 md:hidden"><button type="button" onClick={() => setIsMenuOpen(true)} className="shrink-0 text-dark-700" aria-label="Open menu"><FaBars size={20} /></button><Link to="/dashboard" className="flex items-center gap-2 text-lg font-bold text-primary-700" aria-label="Compassion International"><FaHandsHelping /><span>Compassion International</span></Link></div>
