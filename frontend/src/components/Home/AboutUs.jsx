...
const AboutUs = () => {
  ...
  const saveWorker = async (event) => {
    ...
    if (!editingId && !file) {
      toast.error('A worker photo is required.');
      return;
    }
    setSaving(true);
    try {
      ...
      if (file) {
        const response = await mediaService.upload(file, { title: form.name, description: `${form.role} photo` });
      ...
      resetForm();
    } catch (error) { console.error('Worker save error:', error); } finally { setSaving(false); }
  };

  const deleteWorker = async (id) => {
    if (!window.confirm('Delete this worker permanently?')) return;
    try { await workerService.delete(id); setWorkers((current) => current.filter((worker) => worker._id !== id)); toast.success('Worker deleted.'); } catch (error) { console.error('Worker delete error:', error); }
  };
