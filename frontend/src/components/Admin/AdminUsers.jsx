  ...
  return (
    <div className="container-custom py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.2fr)]">
        <form onSubmit={createUser} className="card space-y-4 p-6"><h2 className="flex items-center gap-2 font-semibold text-dark-900"><FaUserPlus className="text-primary-600" /> Create viewer</h2><input name="name" value={form.name} onChange={handleChange} required className="input-field" placeholder="Viewer name" /><input name="email" type="email" value={form.email} onChange={handleChange} required className="input-field" placeholder="Email address" /><input name="password" type="password" minLength="6" value={form.password} onChange={handleChange} required className="input-field" placeholder="Temporary password" /><button disabled={saving} className="btn-primary flex w-full items-center justify-center gap-2 disabled:opacity-50">{saving ? 'Creating...' : 'Create viewer'}</button></form>
