...
import { FaEye, FaEyeSlash, FaTrash, FaUserPlus, FaUserShield, FaUsers } from 'react-icons/fa';
...
const AdminUsers = () => {
  ...
  const promoteUser = async (item) => {
    if (!window.confirm(`Make ${item.name} an administrator?`)) return;
    try {
      const response = await adminService.promoteUser(item._id);
      setUsers((current) => current.map((userItem) => userItem._id === item._id ? response.data.user : userItem));
      toast.success(`${item.name} is now an administrator.`);
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to promote user.');
    }
  };
  ...
  return (
    <div className="container-custom py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.2fr)]">
        ...
        <section className="card overflow-hidden"><div className="border-b border-dark-100 px-6 py-4"><h2 className="font-semibold text-dark-900">Accounts ({users.length})</h2></div><div className="divide-y divide-dark-100">{users.map((item) => <div key={item._id} className="flex items-center justify-between gap-4 px-6 py-4"><div className="min-w-0"><p className="truncate font-semibold text-dark-800">{item.name}</p><p className="truncate text-sm text-dark-500">{item.email}</p><span className="text-xs capitalize text-primary-600">{item.role}</span></div>{item.role !== 'admin' && <div className="flex shrink-0 gap-2"><button type="button" onClick={() => promoteUser(item)} className="rounded-lg p-2 text-primary-600 hover:bg-primary-50" aria-label={`Make ${item.name} an administrator`} title="Make administrator"><FaUserShield /></button><button type="button" onClick={() => removeUser(item._id)} className="rounded-lg p-2 text-red-500 hover:bg-red-50" aria-label={`Remove ${item.name}`} title="Remove viewer"><FaTrash /></button></div>}</div>)}{users.length === 0 && <p className="px-6 py-8 text-sm text-dark-500">No accounts found.</p>}</div></section>
      </div>
    </div>
  );
};
