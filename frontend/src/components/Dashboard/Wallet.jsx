...
const Wallet = () => {
  ...
  useEffect(() => {
    loadWallet();
    const refreshInterval = setInterval(loadWallet, 30000);
  ...
  const handleTransaction = async (type) => {
    ...
    if (!Number.isInteger(numericAmount) || numericAmount < 1) {
      toast.error('Enter a whole RWF amount of at least 1.');
      return;
    }
    ...
    if (type === 'withdraw' && (!taskDescription || taskDescription.trim().length < 10)) {
      toast.error('Add a short task description before requesting a withdrawal to help prevent fraud.');
      return;
    }
    ...
    try {
      ...
      const payload = type === 'deposit' ? { amount: numericAmount } : { amount: numericAmount, taskDescription: taskDescription.trim() };
      const response = await service(payload);
      setWallet(response.data.wallet);
      setAmount('');
      if (type === 'withdraw') setTaskDescription('');
  ...
  return (
    <div className="container-custom py-8">
      ...
      <section className="card mt-6 p-5">
        ...
        <div className="mt-4 rounded-xl border border-dark-200 bg-dark-50 p-4">
          <label className="mb-2 block text-sm font-medium text-dark-700">Task description for withdrawal review</label>
          <textarea
            value={taskDescription}
            onChange={(event) => setTaskDescription(event.target.value)}
            className="input-field min-h-[90px] resize-none"
            placeholder="Example: I completed a 2-hour creator watch campaign and earned this amount from approved video promotion."
          />
        </div>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row"><input type="number" min="1" step="1" value={amount} onChange={(event) => setAmount(event.target.value)} className="input-field sm:max-w-xs" placeholder="Amount in RWF" /><button type="button" onClick={() => handleTransaction('deposit')} disabled={!!processing} className="btn-secondary inline-flex items-center justify-center gap-2"><FaArrowDown className="text-green-600" />{processing === 'deposit' ? 'Submitting...' : 'I sent the deposit'}</button><button type="button" onClick={() => handleTransaction('withdraw')} disabled={!!processing} className="btn-primary inline-flex items-center justify-center gap-2"><FaArrowUp />{processing === 'withdraw' ? 'Processing...' : 'Withdraw'}</button></div>
      </section>

      <section className="card mt-6 overflow-hidden"><div className="border-b border-dark-100 px-5 py-4"><h2 className="font-semibold text-dark-900">Recent transactions</h2></div>{!wallet?.transactions?.length ? <p className="p-6 text-sm text-dark-500">No wallet transactions yet.</p> : <div className="divide-y divide-dark-100">{wallet.transactions.map((transaction) => { const isCredit = transaction.type === 'deposit' || transaction.type === 'reward'; return <div key={transaction._id} className="flex items-center justify-between px-5 py-4"><div><p className="font-medium capitalize text-dark-800">{transaction.type}</p><p className="text-xs text-dark-500">{new Date(transaction.createdAt).toLocaleString()} · {transaction.status}</p></div><span className={isCredit ? 'font-semibold text-green-600' : 'font-semibold text-primary-600'}>{isCredit ? '+' : '-'}{formatMoney(transaction.amount)}</span></div>; })}</div>}</section>
    </div>
  );
};
