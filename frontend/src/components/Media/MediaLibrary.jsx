...
import { FaDownload, FaEdit, FaImage, FaMicrophone, FaPause, FaPlay, FaPlus, FaShareAlt, FaTrash, FaVideo } from 'react-icons/fa';
...
const MediaLibrary = () => {
  ...
  const shareItem = (item) => shareContent({
    ...
    text: item.description || item.title || item.originalName,
    url: item.videoId ? `${window.location.origin}/video/${item.videoId}` : item.url,
  });
